...
for (d in c(2, 20)) {
    ...
    ts <- list(
        ...
        deadwood:::.oldmst.matrix(X, "euclidean"),
        deadwood::mst(X, distance="euclidean"),
        deadwood::mst(D)
    )
    ...
    for (M in c(1, 2, 10, n-1)) {
        t0 <- deadwood:::.oldmst.matrix(X, M=M, distance="euclidean")
        ...
        ts <- list(
            ...
            deadwood::mst(X, M=M, distance="euclidean"),
            deadwood::mst(D, M=M)
        )
